...
exports.Build = React.createClass({
  addBlock(type) {
    this.props.build.push(new (this.props.build.redactor.getBlock(type))());
  },
  render() {
    return (
      <section className="redactor-build">
        <div className="redactor-build__form">
          <this.props.build.formComponent form={this.props.build.form} />
        </div>
        <div className="redactor-build__blocks">
          <this.props.build.blocks.component collection={this.props.build.blocks} />
        </div>
        <aside className="redactor-adder">
          <span className="redactor-adder__wrapper">
            {
              this.props.build.blockTypes.map(block => {
                let blockClass = this.props.build.redactor.getBlock(block);
