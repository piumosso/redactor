...
exports.Block = React.createClass({
  positionAdder(event) {
    var blockDomNode = React.findDOMNode(this.refs.block);
    var mouseOffset = blockDomNode.offsetParent.offsetTop - event.pageY;
    var blockBomNodeOffset = blockDomNode.offsetTop;
    var blockBomNodeHeight = blockDomNode.clientHeight;

    if (mouseOffset > blockBomNodeHeight / 2) {
      this.props.positionAdder(blockBomNodeOffset + blockBomNodeHeight);
    } else {
      this.props.positionAdder(blockBomNodeOffset);
    }
  },
  render() {
    return (
