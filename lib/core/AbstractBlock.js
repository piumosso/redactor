...
AbstractBlock.prototype.getPrintTemplate = function(context){
    ...
    console.log('printTemplateDir', printTemplateDir);
    fullTemplateFileName = _.compact([this.getType(), context, 'print', 'jade']).join('.');
    console.log('fullTemplateFileName', fullTemplateFileName);
...
AbstractBlock.prototype.print = function(context){
    ...
    if (this.hasBuild) {
        contextD = this.innerBuild.print(context).then(function(innerBuildHtml){
            return Q(
                _.extend(
                    that.model.toJSON(), {
