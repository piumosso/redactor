...
Build.prototype._templateGetter = function(template, templateSource, defaultTemplate, defaultTemplateSource){
    ...
    if (template) {
        ...
        loadedTemplate = fs.readFileSync(__dirname + '/../templates/' + templateSource);
        d.resolve(loadedTemplate);
    } else if (defaultTemplate) {
        d.resolve(defaultTemplate);
    } else if (defaultTemplateSource) {
        loadedTemplate = fs.readFileSync(__dirname + '/../templates/' + defaultTemplateSource);
        d.resolve(loadedTemplate);
    } else {
 ...
 * @param context   Строковый идентификатор контекста печати
 */
Build.prototype.print = function(context){
    var that = this;
    

    return this.getPrintTemplate(context).then(function(printTemplateHtml){
        var printTemplate;

        printTemplate = new (templates.getEngine())(printTemplateHtml);

        return that.blockCollection.print(context).then(function(blocksHtml){
